...
resource "google_redis_instance" "main" {
  name               = "redis${var.region}${var.env}${var.system}${var.component}${var.append_name}"
  memory_size_gb = 1
}
...
resource "kubernetes_deployment" "vote-front" {
  ...
  spec {
    ...
    template {
      ...
      spec {
        container {
          image = "neilpeterson/vote-front:v3"
          name  = "vote-front"
          ...
          env {
            name  = "VOTE1VALUE"
            value = "${var.vote1value}"
          }
          env {
            name  = "VOTE2VALUE"
            value = "${var.vote2value}"
          }
...
resource "kubernetes_service" "vote-front" {
  ...
  spec {
    selector = {
      app = "${kubernetes_deployment.vote-front.metadata.0.labels.app}"
    }
